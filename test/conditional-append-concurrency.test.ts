...
describe("insertEntryIfVerifiedHead — real cross-process concurrency", () => {
  it(
    "two independent OS processes racing on the same expectedHead: exactly one is appended, the other gets a conflict, " +
      "exactly one new row exists, and the final chain head agrees with the witness",
    async () => {
      ...
      // base + exactly one racer — the losing racer wrote nothing.
